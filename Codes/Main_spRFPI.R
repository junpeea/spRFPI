...
for (k in 1:nsim){
  ...
  for(l in 1:length(m.cand)){
    rf.gls <- RFGLS_estimate_spatial(s.train.0, y = Y.train.0, X = X.train.0, ntree = ntree.gls, param_estimate = TRUE, nthsize = ns, h=16, mtry = m.cand[l], nrnodes = ms)
    ...
    rf <- randomForest::randomForest(X.train.0, Y.train.0, keep.inbag = TRUE, proximity = TRUE, mtry = m.cand[l], maxnodes = ms)
  ...
  rf.gls <- RFGLS_estimate_spatial(s.train, y = Y.train, X = X.train, ntree = ntree.gls, param_estimate = TRUE, nthsize = ns, h=16, mtry = mtry1, nrnodes = ms)
  ...
  rf <- randomForest::randomForest(X.train, Y.train, keep.inbag = TRUE, proximity = TRUE, mtry = mtry2, nodesize=ns, maxnodes = ms)
  ...
  alter.output <- rfinterval(y~., train_data = train_data, test_data = test_data,
                             ...
                             symmetry = TRUE ,alpha = alpha, params_ranger=list(mtry=mtry2, min.node.size=ns, max.depth=ms))
