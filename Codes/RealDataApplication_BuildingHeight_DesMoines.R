...
mydat =  data.frame(read_excel("DesMoines_Statistic.xlsx"))
mydat$x = (mydat$x - mean(mydat$x))/sd(mydat$x)
mydat$y = (mydat$y - mean(mydat$y))/sd(mydat$y)
mydat$Height <- log(mydat$Height)
mydat$Near_DIST <- log(mydat$Near_DIST+1e-06)
mydat$MBG_Length <- log(mydat$MBG_Length+1e-06)
mydat$MBG_Width <- log(mydat$MBG_Width+1e-06)

alpha = 0.1
ntrain = 1000
ntree.gls <- 10; ntree <- 10
ns = 900
adj = 1.0
eta = 25
nsim  = 30
...
for(sim in 1:nsim){
  ...
  X.train    = train_data[,c(-1,-4)] %>% as.matrix; X.test    = test_data[,c(-1,-4)] %>% as.matrix
  Y.train    = train_data[,c(4)]; Y.test    = test_data[,c(4)]
  ...
  alter.output <- rfinterval(Height~., train_data = train_data[,-1], test_data = test_data[,-1],
...
# save.image(file=paste0(Sys.Date(),"_DesMoines.Rdata"))
