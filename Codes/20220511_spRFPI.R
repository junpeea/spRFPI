...
nu0 = 0.5; phi = 1
ntree.gls <- 0.25*ntrain; 0.25*ntrain
...
for (k in 1:nsim){
  ...
  for(l in 1:length(m.cand)){
    rf.gls <- RFGLS_estimate_spatial(s.train.0, y = Y.train.0, X = X.train.0, ntree = ntree.gls, param_estimate = TRUE, nthsize = ns, h=4, mtry = m.cand[l])
  ...
  # rf.gls <- RFGLS_estimate_spatial(s.train, y = Y.train, X = X.train, ntree = ntree.gls, param_estimate = TRUE, nthsize = ns, h=4, mtry = mtry1)
  # rf.oracle.gls <- RFGLS_estimate_spatial(s.train, y = Y.train, X = X.train, ntree = ntree.gls, param_estimate = FALSE, nthsize = ns, h=4, mtry = mtry1,
...
output.ais.mat = data.frame(oobg.ais.mar,oobgk.ais.mar,oobw.ais.mar,oob.ais.mar,sc.ais.mar,lscp.ais.mar,qrf.ais.mar)
colnames(output.cpr.mat) <- c("OOBG","OOBG-KER","OOBW","OOB","CP","LSCP","QRF")
colnames(output.len.mat) <- c("OOBG","OOBG-KER","OOBW","OOB","CP","LSCP","QRF")
colnames(output.ais.mat) <- c("OOBG","OOBG-KER","OOBW","OOB","CP","LSCP","QRF")

paste0(100*round(colMeans(output.cpr.mat,na.rm=T),3),"(",round(colMeans(output.len.mat,na.rm=T),2),")","[",round(colMeans(output.ais.mat,na.rm=T),2),"]")
