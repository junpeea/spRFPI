...
for (k in 1:nsim){
  ...
  rf <- randomForest::randomForest(X.train, Y.train, keep.inbag = TRUE, proximity = TRUE, mtry = mtry2, nodesize=ns)
  ...
  alter.output <- rfinterval(y~., train_data = train_data, test_data = test_data,
                             ...
                             symmetry = TRUE ,alpha = alpha, params_ranger = list(mtry=mtry2,min.node.size=ns))
...
# png(filename=paste0(Sys.Date(),"LINEAR_Visualization.png"),width=720,height=720)
