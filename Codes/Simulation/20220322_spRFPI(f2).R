...
f4 = function(x){
  # 'x' is a vector, covariates corresponding to a case
  EY = 0.1*(10*(sin(pi*x[1]*x[2]) + 20*(x[3]-0.5)^2 + 10*x[4] + 5*x[5]))
  return(EY)
}
#########
...
# AIS2.vec = rep(NA,length(m.cand))
# PMSPE.vec = rep(NA,length(m.cand))
# CpabsBias.vec = rep(NA,length(m.cand))
...
#     mean( (oobgk.output.list[[t]]$upper_0.1 - oobgk.output.list[[t]]$lower_0.1) + (2/alpha) * (sapply(oobgk.output.list[[t]]$lower_0.1 - Y.train.v, function(w){max(w,0)}) + sapply(Y.train.v - oobgk.output.list[[t]]$upper_0.1, function(w){max(w,0)})), na.rm=T )
#   }) %>% unlist
...
#   oobw.output <- quantForestError(rf, X.train.0, X.train.v,
#                                   train_nodes = train_nodes, alpha = alpha)$estimate
#   AIS2.vec[l]      = mean( (oobw.output$upper_0.1 - oobw.output$lower_0.1) + (2/alpha) * (sapply(oobw.output$lower_0.1 - Y.train.v, function(w){max(w,0)}) + sapply(Y.train.v - oobw.output$upper_0.1, function(w){max(w,0)})), na.rm=T )
#   PMSPE.vec[l]     = mean((Y.train.v - oobw.output$pred)^2)
#   CpabsBias.vec[l] = abs((1-alpha) - mean(oobw.output$lower_0.1 < Y.train.v & oobw.output$upper_0.1 > Y.train.v))
#   cat(paste(Sys.time(),l), AIS.mat[l,], "\n")
#   cat(paste(Sys.time(),l), AIS2.vec, "\n")
# }
# mtry1 = m.cand[which(AIS.mat == min(AIS.mat), arr.ind = TRUE)[1]];mtry1
# mtry2 = m.cand[order(AIS2.vec)[1]];mtry2
...
oobg.cpr.mar   = rep(NA,nsim);oobg.len.mar   = rep(NA,nsim);
oobgk.cpr.mar   = rep(NA,nsim);oobgk.len.mar   = rep(NA,nsim);
oobw.cpr.mar   = rep(NA,nsim);oobw.len.mar   = rep(NA,nsim);
oob.cpr.mar    = rep(NA,nsim);oob.len.mar    = rep(NA,nsim);
sc.cpr.mar     = rep(NA,nsim);sc.len.mar     = rep(NA,nsim);
qrf.cpr.mar    = rep(NA,nsim);qrf.len.mar    = rep(NA,nsim);
lscp.cpr.mar    = rep(NA,nsim);lscp.len.mar     = rep(NA,nsim);
oobgk.ais.mar   = rep(NA,nsim); lscp.ais.mar  = rep(NA,nsim); oobw.ais.mar  = rep(NA,nsim);
...
m.cand = seq(15,20,by = 1)
adj.vec = seq(1,10,by = 0.2)
...
for (k in 1:nsim){
  ...
  mtry1 = m.cand[which(AIS.mat == min(AIS.mat), arr.ind = TRUE)[1]];mtry1
  mtry2 = m.cand[order(AIS2.vec)[1]];mtry2
  adjk   = adj.vec[which(AIS.mat == min(AIS.mat), arr.ind = TRUE)[2]];adjk
  # CpabsBias.vec = CpabsBias.mat[which(AIS.mat == min(AIS.mat), arr.ind = TRUE)[1],]
  # adjk   = adj.vec[order(CpabsBias.vec)[1]];adjk
  
  # rf.gls <- RFGLS_estimate_spatial(s.train.0, y = Y.train.0, X = X.train.0, ntree = ntree.gls, param_estimate = TRUE, nthsize = ns, h=16, mtry = mtry1)
  # oobgk.output.list <- quantGLS(rf.gls, X.train.0, X.train.v, s.train.v, alpha = alpha, adj.vec)
  # AIS.vec = lapply(c(1:length(oobgk.output.list)), function(t){
  #   mean( (oobgk.output.list[[t]]$upper_0.1 - oobgk.output.list[[t]]$lower_0.1) + (2/alpha) * (sapply(oobgk.output.list[[t]]$lower_0.1 - Y.train.v, function(w){max(w,0)}) + sapply(Y.train.v - oobgk.output.list[[t]]$upper_0.1, function(w){max(w,0)})), na.rm=T )
  # }) %>% unlist
  # CpabsBias.vec = lapply(c(1:length(oobgk.output.list)), function(t){
  #   abs((1-alpha) - mean(oobgk.output.list[[t]][,4] < Y.train.v & oobgk.output.list[[t]][,5] > Y.train.v))
  # }) %>% unlist
  # # adjk   = adj.vec[order(CpabsBias.vec)[1]];adjk
  # adjk   = adj.vec[order(AIS.vec)[1]];adjk
  ...
  oobg.output <- quantGLSForestError(rf.gls, X.train, X.test, alpha = alpha, kernel=FALSE)
  ...
  oobgk.ais.mar[k] = mean( (oobgk.output$upper_0.1 - oobgk.output$lower_0.1) + (2/alpha) * (sapply(oobgk.output$lower_0.1 - Y.test, function(w){max(w,0)}) + sapply(Y.test - oobgk.output$upper_0.1, function(w){max(w,0)})), na.rm=T )
  ...
  boxplot(lendmat[,ind],ylim=c(0,2*max(Y.test)),xlab="Test Index",ylab="Width",xaxt="n",col=scales::alpha(4,0.25))
  par(new=T);boxplot(lendmat2[,ind],ylim=c(0,2*max(Y.test)),xaxt="n",col=scales::alpha(2,0.25),xlab="",ylab="")
  par(new=T);boxplot(lendmat3[,ind],ylim=c(0,2*max(Y.test)),xaxt="n",col=scales::alpha(3,0.25),xlab="",ylab="")
  par(new=TRUE)
...
output.cpr.mat = data.frame(oobg.cpr.mar,oobgk.cpr.mar,oobw.cpr.mar,oob.cpr.mar,sc.cpr.mar,lscp.cpr.mar,qrf.cpr.mar)
output.len.mat = data.frame(oobg.len.mar,oobgk.len.mar,oobw.len.mar,oob.len.mar,sc.len.mar,lscp.len.mar,qrf.len.mar)
colnames(output.cpr.mat) <- c("OOBG","OOBG-KER","OOBW","OOB","CP","LSCP","QRF")
colnames(output.len.mat) <- c("OOBG","OOBG-KER","OOBW","OOB","CP","LSCP","QRF")

paste0(100*round(colMeans(output.cpr.mat,na.rm=T),3),"(",round(colMeans(output.len.mat,na.rm=T),2),")")


# png(filename=paste0(Sys.Date(),"SINUSOIDAL_Visualization.png"),width=720,height=720)
