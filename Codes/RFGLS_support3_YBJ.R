...
quantGLS = function(rf.gls, X.train, X.test, s.test, alpha, adj.vec){
  ...
  output_df_list <- lapply(adj.vec,function(adjust){
    # format pred2iction interval output
    percentiles <- sort(c(alpha / 2, 1 - (alpha / 2)))
    interval_col_names <- paste0(rep(c("lower_", "upper_"), each = length(alpha)),
                                 c(alpha, rev(alpha)))
    
    col_names <- c("pred", "mspe", "bias", interval_col_names)
    
    oob_error_stats[, (interval_col_names) :=  lapply(percentiles, FUN = function(p){
      pred + lapply(c(1:ntest),function(k){
        bins = seq(min(oob_error_stats$all_errs[[k]],na.rm=T)*3,max(oob_error_stats$all_errs[[k]],na.rm=T)*3,length.out=1000)
